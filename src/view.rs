...
impl<T: BufRead> View<T> {
    ...
    pub fn new(model: GameBoard, reader: T) -> Self {
        ...
        // cannot truncate ∵ both `model.rows()` and `model.columns()` values are <= 16 bits
    ...
    fn col_headers(&self) -> String {
        ...
        // cannot overflow ∵ `model.columns()` is bound to 16 bits which limits `max_row_digits` to 5 (3 bits).
        ...
        (1..=self.max_col_digits).rev()
                                 .for_each(|p| {
                                     // cannot truncate ∵ `p` cannot exceed 5 (# of digits in base 10 u16::MAX).
                                     #[allow(clippy::cast_possible_truncation)]
                                         let pow_10 = 10_usize.pow(p as u32);
                                     result += &(" ".repeat(self.max_row_digits) + "|");
                                     // cannot overflow ∵ `model.colums()` is bound to 16 bits, which, in turn limits
                                     // `max_row_digits` to 5 (3 bits).
                                     // *1 cannot truncate ∵ `d % 10` never exceeds 9.
                                         #[allow(clippy::integer_arithmetic, clippy::cast_possible_truncation)]
                                     (1..(self.max_row_digits + self.model.cols())).for_each(|c| {
                                         result += &match c % pow_10 {
                                             // *1
                                             d if d > 0 => char::from_digit((d % 10) as u32, BASE_10)
